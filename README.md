...
只安装 `daed` 或 `luci-app-daed` 软件包不足以保证正常运行，还需要在对应机型的编译配置中开启 BTF、BPF/XDP 和 IPv4 策略路由支持：
...
```text
luci-app-daed daed daed-geoip daed-geosite luci-i18n-daed-zh-cn kmod-sched-core kmod-sched-bpf kmod-veth
```

其中 `CONFIG_KERNEL_DEBUG_INFO_REDUCED` 必须关闭，`CONFIG_KERNEL_DEBUG_INFO_BTF` 必须开启，否则 daed 可能因缺少 eBPF CO-RE/BTF 环境而启动失败。
---
## 支持的设备型号
